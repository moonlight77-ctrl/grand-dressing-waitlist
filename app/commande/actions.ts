...
export async function updateOrderStatus(
  ...
  if (timestampField[status]) updates[timestampField[status]] = new Date().toISOString();

  if (status === 'returned') {
    const serviceClient = getServiceClient();
    const { data: order } = await supabase.from('orders').select('user_id, total_capacity_cost').eq('id', orderId).single();
    if (order) {
      const { data: profile } = await serviceClient.from('profiles').select('style_points_used').eq('id', order.user_id).single();
      if (profile) {
